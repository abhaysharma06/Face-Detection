...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text):
    ...
    for (x, y, w, h) in features:
        ...
        cv2.putText(img, text, (x, y-4), cv2.FONT_HERSHEY_SIMPLEX,1, color, 2, cv2.LINE_AA)
        coords = [x, y, w, h]
    return coords, img


# Method to detect the features
def detect(img, faceCascade,eyeCascade,  outhCascade):
    color = {"blue":(255,0,0), "red":(0,0,255), "green":(0,255,0), "white":(255,255,255)}
    coords, img = draw_boundary(img, faceCascade, 1.1, 10, color['white'], "face")
